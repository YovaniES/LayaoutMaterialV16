...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  private handleError(error: HttpErrorResponse) {
    if (this.errorPages.includes(error.status)) {
      ...
      if (
        error.error.detalle ===
        '400 BAD_REQUEST "Error de validación. Uno o mas nombres de numEmpaque ya existen registrados, estos deben ser unicos."'
      ) {
        this.toast.error('ERROR: Uno o más paquetes ya existen registrados, deben ser únicos');
      } else {
